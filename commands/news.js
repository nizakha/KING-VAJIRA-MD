...
cmd({ 
     ...
     async(Void, citel) => { 
        let res = await api.latest_id(); 
        const nid = res.results.derana_id; 
        let derana = await api.derana(nid); 
        const tt = derana.results.TITLE; 
        const dss = derana.results.DESCRIPTION; 
        const ttime = derana.results.PUBLISHED; 
        const img = derana.results.COVER; 
        const cap = `📃ＮＥＷＳ ＢＹ ＶＡＪＩＲＡ\n\n◉───────────────\n🏷️ ᴛɪᴛʟᴇ➨ ${tt}\n◉───────────────\n 🕒 ᴛɪᴍᴇ➨ ${ttime}\n◉─────────────── \n 📑 ᴅᴇꜱᴄʀɪᴘᴛɪᴏɴ➨ ${dss}\n◉─────────────── \n\n ▶.GENERATED BY VAJIRA.◀`; 
                 await Void.sendMessage(citel.chat,{image:{url: img}, caption: cap})
 })
